...
if menu == get_text("🏠 Home", "🏠 होम"):
    ...
    st.markdown(
        f"<div style='background-color: #d9ed92; padding:10px; border-radius:8px'>"
        f"<b>{get_text('Be vigilant, be safe.', 'सतर्क रहें, सुरक्षित रहें।')}</b></div>",
        unsafe_allow_html=True
    )
    st.write(get_text(
        "Verify, report, and empower safety in your locality.",
